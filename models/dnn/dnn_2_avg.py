"""Closest model to DNN-2 average."""
##############################################################################
# LIBS
##############################################################################
import pandas as pd
from pathlib import Path
from dnn_2.dnn_2_best_model_training_function import train_test
...
# OUTPUTS
##############################################################################
prediction_df = pd.DataFrame({'actual': y_test, 'prediction': y_test_pred, 'Model': 'DNN-2'})
error_df = pd.DataFrame({'error': y_test_pred - y_test, 'Model': 'DNN-2'})

write_csv = False
if write_csv:
    prediction_df.to_csv('test_predictions_dnn_2.csv', index=False)
    error_df.to_csv('error_df_dnn_2.csv', index=False)
    hist.to_csv('dnn_2_training_history.csv', index=False)
