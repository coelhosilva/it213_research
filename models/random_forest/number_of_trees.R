...
rf_1_tree_performance <- data.frame(n_trees = n_trees, rmse = rmse_rf_1)
rf_1_tree_performance$Model <- "RF-1"

rf_2_tree_performance <- data.frame(n_trees = n_trees, rmse = rmse_rf_2)
rf_2_tree_performance$Model <- "RF-2"

tree_performance <- bind_rows(rf_1_tree_performance, rf_2_tree_performance)

saveRDS(tree_performance, here('models/random_forest/results/number_of_trees_performance.RDS'))
