...
# MODEL FITTING
model_gravity <- glm(
  viagensTotais ~ log(pop_origem)+log(pop_destino)+log(ppc_origem)+log(ppc_destino)+log(distancia)+log(THEIL_origem)+log(THEIL_destino)+log(GINI_origem)+log(GINI_destino),
...
predict_gravity <- function(model, dataset) {
  ...
  y_pred <- (
    exp(k)*
      ...
      exp(beta_6*log(dataset$THEIL_origem))*
      exp(beta_7*log(dataset$THEIL_destino))*
      exp(beta_8*log(dataset$GINI_origem))*
      exp(beta_9*log(dataset$GINI_destino))
    )
...
if(save_model_error) {
  error_actual_df$model <- "Gravity-2"
  saveRDS(error_actual_df, "error_df_gravity2.RDS")
}